...
def _insert_ref():
    ...
    try:
        for index, programme in data_frame.iterrows():
            commune = session.query(_Commune).filter_by(**{'code_commune': programme['insee_com']}).one_or_none()
            if commune is None:
                commune = _Commune(code_commune=programme['insee_com'], code_crte=programme["id_crte"],
                ...
                logger.info('Ignore ligne %s commune %s in %s', index, programme['insee_com'], programme["id_crte"])
            session.commit()
