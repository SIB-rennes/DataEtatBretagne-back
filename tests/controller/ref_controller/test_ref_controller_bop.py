...
def test_bop_by_code(test_client, add_data):
    code = "BOP2"
    resp = test_client.get('/referentiels/api/v1/bop/'+code)
...
def test_centre_bop_not_found(test_client, add_data):
    # GIVEN
    code_not_found = 'code_not_found'
    resp = test_client.get('/referentiels/api/v1/bop/'+code_not_found)
    assert resp.status_code == 404


def test_search_bop_no_content(test_client, add_data):
    test="fcode1"
    resp = test_client.get('/referentiels/api/v1/bop?query='+test)
